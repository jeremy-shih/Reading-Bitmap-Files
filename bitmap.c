...
void read_bitmap_metadata(FILE *image, int *pixel_array_offset, int *width, int *height) {
    // find where fixel array starts and read in
    fseek(image, 10, SEEK_SET);
    fread(pixel_array_offset, 4, 1, image);

    // find where width starts and read in
    fseek(image, 18, SEEK_SET);
    fread(width, 4, 1, image);

    // find where height starts and read in
...
struct pixel **read_pixel_array(FILE *image, int pixel_array_offset, int width, int height) {
    struct pixel **read = malloc(sizeof(struct pixel *) * height);
    for (int i= 0; i < height; i++){
        read[i] = malloc(sizeof(struct pixel) * width);
    }

    fseek(image, pixel_array_offset, SEEK_SET);
    for (int i = 0; i < height; i++){
        for (int j = 0; j < width; j++){
            fread(&(read[i][j].blue), 1, 1, image);
            fread(&(read[i][j].green), 1, 1, image);
            fread(&(read[i][j].red), 1, 1, image);
        }
    }
    return read;
}
